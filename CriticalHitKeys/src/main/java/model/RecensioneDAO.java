...
public class RecensioneDAO {
    public void doSave(Recensione r) {
        try(Connection conn = ConPool.getConnection()){
            PreparedStatement ps = conn.prepareStatement("INSERT INTO Recensione (ID_Recensione, Voto, Descrizione_Rec, Username_Ut, Email_Ut) VALUES (?, ?, ?, ?, ?)");
            ps.setInt(1, r.getID_Recensione());
            ps.setInt(2, r.getVoto());
            ps.setString(3, r.getDescrizione_Rec());
            ps.setString(4, r.getUsername_Ut());
            ps.setString(5, r.getEmail_Ut());
